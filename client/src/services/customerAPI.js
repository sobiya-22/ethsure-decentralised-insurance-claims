...
export const getCustomer = async (wallet_address) => {
  const res = await api.get("/api/customer/get", { wallet_address });
  return res;
};

// Update customer details
export const updateCustomer = async (wallet_address, updateData) => {
  const res = await api.patch("/api/customer/update", { wallet_address, updateData });
  return res;
};
...
export const checkCustomerKYCStatus = async (wallet_address) => {
  const res = await api.get("/api/customer/kyc-status", { wallet_address });
  return res;
};
...
export const getAllCustomers = async () => {
  const res = await api.get("/api/customer/all");
  return res;
};
